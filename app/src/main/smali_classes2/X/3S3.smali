.class public LX/3S3;
.super Ljava/lang/Object;
.source ""

# interfaces
.implements LX/43P;
.implements LX/43Q;
.implements LX/49l;


# instance fields
.field public final A00:LX/1QX;

.field public final A01:LX/2li;

.field public final A02:LX/2yp;


# direct methods
.method public constructor <init>(LX/1QX;LX/2li;LX/2yp;)V
    .locals 0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-object p1, p0, LX/3S3;->A00:LX/1QX;

    iput-object p3, p0, LX/3S3;->A02:LX/2yp;

    iput-object p2, p0, LX/3S3;->A01:LX/2li;

    return-void
.end method


# virtual methods
.method public Arg(LX/2qc;LX/373;)V
    .locals 9

    instance-of v0, p2, LX/1hN;

    if-eqz v0, :cond_f

    check-cast p2, LX/1hN;

    iget-object v4, p1, LX/2qc;->A00:LX/1B3;

    iget-object v0, v4, LX/6fq;->A00:LX/6fI;

    check-cast v0, LX/1FR;

    iget-object v0, v0, LX/1FR;->productMessage_:LX/1ER;

    if-nez v0, :cond_0

    sget-object v0, LX/1ER;->DEFAULT_INSTANCE:LX/1ER;

    :cond_0
    invoke-virtual {v0}, LX/6fI;->A0H()LX/6fq;

    move-result-object v3

    check-cast v3, LX/18M;

    iget-object v0, v4, LX/6fq;->A00:LX/6fI;

    check-cast v0, LX/1FR;

    iget-object v0, v0, LX/1FR;->productMessage_:LX/1ER;

    if-nez v0, :cond_1

    sget-object v0, LX/1ER;->DEFAULT_INSTANCE:LX/1ER;

    :cond_1
    iget-object v0, v0, LX/1ER;->product_:LX/1Es;

    if-nez v0, :cond_2

    sget-object v0, LX/1Es;->DEFAULT_INSTANCE:LX/1Es;

    :cond_2
    invoke-virtual {v0}, LX/6fI;->A0H()LX/6fq;

    move-result-object v2

    iget-object v1, p0, LX/3S3;->A01:LX/2li;

    iget-object v0, v2, LX/6fq;->A00:LX/6fI;

    check-cast v0, LX/1Es;

    iget-object v0, v0, LX/1Es;->productImage_:LX/1FB;

    if-nez v0, :cond_3

    sget-object v0, LX/1FB;->DEFAULT_INSTANCE:LX/1FB;

    :cond_3
    invoke-virtual {v0}, LX/6fI;->A0H()LX/6fq;

    move-result-object v0

    check-cast v0, LX/1Ab;

    invoke-virtual {v1, p1, v0, p2}, LX/2li;->A01(LX/2qc;LX/1Ab;LX/1hX;)LX/1Ab;

    move-result-object v8

    if-eqz v8, :cond_e

    iget-object v0, p2, LX/1hN;->A01:Lcom/whatsapp/jid/UserJid;

    if-eqz v0, :cond_e

    invoke-static {v3, v0}, LX/0yK;->A0m(LX/6fq;Lcom/whatsapp/jid/Jid;)Ljava/lang/String;

    move-result-object v5

    iget-object v1, v3, LX/6fq;->A00:LX/6fI;

    check-cast v1, LX/1ER;

    invoke-virtual {v5}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget v0, v1, LX/1ER;->bitField0_:I

    or-int/lit8 v0, v0, 0x2

    iput v0, v1, LX/1ER;->bitField0_:I

    iput-object v5, v1, LX/1ER;->businessOwnerJid_:Ljava/lang/String;

    iget-object v0, p2, LX/1hN;->A06:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_4

    iget-object v5, p2, LX/1hN;->A06:Ljava/lang/String;

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1Es;

    invoke-virtual {v5}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget v0, v1, LX/1Es;->bitField0_:I

    or-int/lit8 v0, v0, 0x2

    iput v0, v1, LX/1Es;->bitField0_:I

    iput-object v5, v1, LX/1Es;->productId_:Ljava/lang/String;

    :cond_4
    iget-object v0, p2, LX/1hN;->A04:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_5

    iget-object v5, p2, LX/1hN;->A04:Ljava/lang/String;

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1Es;

    invoke-virtual {v5}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget v0, v1, LX/1Es;->bitField0_:I

    or-int/lit8 v0, v0, 0x8

    iput v0, v1, LX/1Es;->bitField0_:I

    iput-object v5, v1, LX/1Es;->description_:Ljava/lang/String;

    :cond_5
    iget-object v0, p2, LX/1hN;->A09:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_6

    iget-object v5, p2, LX/1hN;->A09:Ljava/lang/String;

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1Es;

    invoke-virtual {v5}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget v0, v1, LX/1Es;->bitField0_:I

    or-int/lit8 v0, v0, 0x4

    iput v0, v1, LX/1Es;->bitField0_:I

    iput-object v5, v1, LX/1Es;->title_:Ljava/lang/String;

    :cond_6
    iget-object v0, p2, LX/1hN;->A02:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_7

    iget-object v5, p2, LX/1hN;->A02:Ljava/lang/String;

    invoke-static {v3}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1ER;

    invoke-virtual {v5}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget v0, v1, LX/1ER;->bitField0_:I

    or-int/lit8 v0, v0, 0x8

    iput v0, v1, LX/1ER;->bitField0_:I

    iput-object v5, v1, LX/1ER;->body_:Ljava/lang/String;

    :cond_7
    iget-object v0, p2, LX/1hN;->A05:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_8

    iget-object v5, p2, LX/1hN;->A05:Ljava/lang/String;

    invoke-static {v3}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1ER;

    invoke-virtual {v5}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget v0, v1, LX/1ER;->bitField0_:I

    or-int/lit8 v0, v0, 0x10

    iput v0, v1, LX/1ER;->bitField0_:I

    iput-object v5, v1, LX/1ER;->footer_:Ljava/lang/String;

    :cond_8
    iget-object v0, p2, LX/1hN;->A03:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_9

    iget-object v0, p2, LX/1hN;->A0A:Ljava/math/BigDecimal;

    if-eqz v0, :cond_9

    iget-object v5, p2, LX/1hN;->A03:Ljava/lang/String;

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1Es;

    invoke-virtual {v5}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget v0, v1, LX/1Es;->bitField0_:I

    or-int/lit8 v0, v0, 0x10

    iput v0, v1, LX/1Es;->bitField0_:I

    iput-object v5, v1, LX/1Es;->currencyCode_:Ljava/lang/String;

    iget-object v0, p2, LX/1hN;->A0A:Ljava/math/BigDecimal;

    sget-object v7, LX/2vn;->A00:Ljava/math/BigDecimal;

    invoke-virtual {v0, v7}, Ljava/math/BigDecimal;->multiply(Ljava/math/BigDecimal;)Ljava/math/BigDecimal;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/Number;->longValue()J

    move-result-wide v0

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v6

    check-cast v6, LX/1Es;

    iget v5, v6, LX/1Es;->bitField0_:I

    or-int/lit8 v5, v5, 0x20

    iput v5, v6, LX/1Es;->bitField0_:I

    iput-wide v0, v6, LX/1Es;->priceAmount1000_:J

    iget-object v1, p2, LX/1hN;->A0B:Ljava/math/BigDecimal;

    if-eqz v1, :cond_9

    sget-object v0, Ljava/math/BigDecimal;->ZERO:Ljava/math/BigDecimal;

    invoke-virtual {v0, v1}, Ljava/math/BigDecimal;->compareTo(Ljava/math/BigDecimal;)I

    move-result v0

    if-eqz v0, :cond_9

    iget-object v0, p2, LX/1hN;->A0B:Ljava/math/BigDecimal;

    invoke-virtual {v0, v7}, Ljava/math/BigDecimal;->multiply(Ljava/math/BigDecimal;)Ljava/math/BigDecimal;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/Number;->longValue()J

    move-result-wide v0

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v6

    check-cast v6, LX/1Es;

    iget v5, v6, LX/1Es;->bitField0_:I

    or-int/lit16 v5, v5, 0x400

    iput v5, v6, LX/1Es;->bitField0_:I

    iput-wide v0, v6, LX/1Es;->salePriceAmount1000_:J

    :cond_9
    iget-object v0, p2, LX/1hN;->A08:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_a

    iget-object v5, p2, LX/1hN;->A08:Ljava/lang/String;

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1Es;

    invoke-virtual {v5}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget v0, v1, LX/1Es;->bitField0_:I

    or-int/lit8 v0, v0, 0x40

    iput v0, v1, LX/1Es;->bitField0_:I

    iput-object v5, v1, LX/1Es;->retailerId_:Ljava/lang/String;

    :cond_a
    iget-object v0, p2, LX/1hN;->A07:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_b

    iget-object v5, p2, LX/1hN;->A07:Ljava/lang/String;

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1Es;

    invoke-virtual {v5}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iget v0, v1, LX/1Es;->bitField0_:I

    or-int/lit16 v0, v0, 0x80

    iput v0, v1, LX/1Es;->bitField0_:I

    iput-object v5, v1, LX/1Es;->url_:Ljava/lang/String;

    :cond_b
    iget v5, p2, LX/1hN;->A00:I

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1Es;

    iget v0, v1, LX/1Es;->bitField0_:I

    or-int/lit16 v0, v0, 0x100

    iput v0, v1, LX/1Es;->bitField0_:I

    iput v5, v1, LX/1Es;->productImageCount_:I

    invoke-static {v2}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1Es;

    invoke-virtual {v8}, LX/6fq;->A05()LX/6fI;

    move-result-object v0

    check-cast v0, LX/1FB;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iput-object v0, v1, LX/1Es;->productImage_:LX/1FB;

    iget v0, v1, LX/1Es;->bitField0_:I

    or-int/lit8 v0, v0, 0x1

    iput v0, v1, LX/1Es;->bitField0_:I

    invoke-static {v3}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1ER;

    invoke-virtual {v2}, LX/6fq;->A05()LX/6fI;

    move-result-object v0

    check-cast v0, LX/1Es;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iput-object v0, v1, LX/1ER;->product_:LX/1Es;

    iget v0, v1, LX/1ER;->bitField0_:I

    or-int/lit8 v0, v0, 0x1

    iput v0, v1, LX/1ER;->bitField0_:I

    iget-object v2, p1, LX/2qc;->A01:LX/3dD;

    iget-object v1, p1, LX/2qc;->A09:[B

    invoke-static {v2, p2, v1}, LX/2yp;->A02(LX/3dD;LX/373;[B)Z

    move-result v0

    if-eqz v0, :cond_c

    iget-object v0, p0, LX/3S3;->A02:LX/2yp;

    invoke-static {p1, v2, p2, v0, v1}, LX/2qc;->A00(LX/2qc;LX/3dD;LX/373;LX/2yp;[B)LX/1FC;

    move-result-object v0

    invoke-static {v3}, LX/0yM;->A0F(LX/6fq;)LX/6fI;

    move-result-object v1

    check-cast v1, LX/1ER;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iput-object v0, v1, LX/1ER;->contextInfo_:LX/1FC;

    iget v0, v1, LX/1ER;->bitField0_:I

    or-int/lit8 v0, v0, 0x20

    iput v0, v1, LX/1ER;->bitField0_:I

    :cond_c
    invoke-static {v4}, LX/0yI;->A0N(LX/6fq;)LX/1FR;

    move-result-object v2

    invoke-virtual {v3}, LX/6fq;->A05()LX/6fI;

    move-result-object v0

    check-cast v0, LX/1ER;

    invoke-virtual {v0}, Ljava/lang/Object;->getClass()Ljava/lang/Class;

    iput-object v0, v2, LX/1FR;->productMessage_:LX/1ER;

    iget v1, v2, LX/1FR;->bitField0_:I

    const/high16 v0, 0x1000000

    or-int/2addr v1, v0

    iput v1, v2, LX/1FR;->bitField0_:I

    :cond_d
    return-void

    :cond_e
    invoke-static {}, LX/001;->A0m()Ljava/lang/StringBuilder;

    move-result-object v1

    const-string v0, "FMessageProduct/buildE2eMessage/unable to send encrypted media message due to missing mediaKey or businessOwnerJid; message.key="

    invoke-static {p2, v0, v1}, LX/373;->A0V(LX/373;Ljava/lang/String;Ljava/lang/StringBuilder;)V

    const-string v0, "; media_wa_type="

    invoke-static {p2, v0, v1}, LX/373;->A0W(LX/373;Ljava/lang/String;Ljava/lang/StringBuilder;)V

    const-string v0, "; business_owner_jid="

    invoke-virtual {v1, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    iget-object v0, p2, LX/1hN;->A01:Lcom/whatsapp/jid/UserJid;

    invoke-static {v0, v1}, LX/0yF;->A1J(Ljava/lang/Object;Ljava/lang/StringBuilder;)V

    iget-object v0, p2, LX/1hN;->A01:Lcom/whatsapp/jid/UserJid;

    if-nez v0, :cond_d

    iget-boolean v0, p1, LX/2qc;->A02:Z

    if-eqz v0, :cond_d

    iget-object v0, p0, LX/3S3;->A00:LX/1QX;

    invoke-static {v0}, LX/2tw;->A0D(LX/2tw;)Z

    move-result v0

    if-eqz v0, :cond_d

    const/4 v0, 0x0

    invoke-static {v0}, LX/1yn;->A00(I)LX/1yn;

    move-result-object v0

    throw v0

    :cond_f
    const-string v0, "FMessageProductProtobuf/not supported message"

    invoke-static {v0}, LX/001;->A0d(Ljava/lang/String;)Ljava/lang/IllegalArgumentException;

    move-result-object v0

    throw v0
.end method

.method public BYe(LX/2sL;)LX/373;
    .locals 6

    iget-object v2, p1, LX/2sL;->A08:LX/1FR;

    iget v1, v2, LX/1FR;->bitField0_:I

    const/high16 v0, 0x1000000

    and-int/2addr v1, v0

    if-eqz v1, :cond_6

    iget-object v0, v2, LX/1FR;->productMessage_:LX/1ER;

    move-object v5, v0

    if-nez v0, :cond_0

    sget-object v0, LX/1ER;->DEFAULT_INSTANCE:LX/1ER;

    :cond_0
    iget v0, v0, LX/1ER;->bitField0_:I

    and-int/lit8 v0, v0, 0x1

    if-eqz v0, :cond_6

    if-nez v5, :cond_1

    sget-object v5, LX/1ER;->DEFAULT_INSTANCE:LX/1ER;

    :cond_1
    invoke-static {v5}, LX/39J;->A06(Ljava/lang/Object;)V

    iget-object v2, p1, LX/2sL;->A0A:LX/30h;

    iget-wide v0, p1, LX/2sL;->A03:J

    new-instance v3, LX/1hN;

    invoke-direct {v3, v2, v0, v1}, LX/1hN;-><init>(LX/30h;J)V

    :try_start_0
    iget-object v0, v5, LX/1ER;->businessOwnerJid_:Ljava/lang/String;

    invoke-static {v0}, Lcom/whatsapp/jid/UserJid;->get(Ljava/lang/String;)Lcom/whatsapp/jid/UserJid;

    move-result-object v0

    iput-object v0, v3, LX/1hN;->A01:Lcom/whatsapp/jid/UserJid;
    :try_end_0
    .catch LX/1z2; {:try_start_0 .. :try_end_0} :catch_0

    :catch_0
    iget-object v0, v3, LX/1hN;->A01:Lcom/whatsapp/jid/UserJid;

    if-eqz v0, :cond_5

    iget-object v4, v5, LX/1ER;->product_:LX/1Es;

    if-nez v4, :cond_2

    sget-object v4, LX/1Es;->DEFAULT_INSTANCE:LX/1Es;

    :cond_2
    iget-object v0, v4, LX/1Es;->productId_:Ljava/lang/String;

    iput-object v0, v3, LX/1hN;->A06:Ljava/lang/String;

    iget-object v0, v4, LX/1Es;->title_:Ljava/lang/String;

    iput-object v0, v3, LX/1hN;->A09:Ljava/lang/String;

    iget-object v0, v5, LX/1ER;->body_:Ljava/lang/String;

    iput-object v0, v3, LX/1hN;->A02:Ljava/lang/String;

    iget-object v0, v5, LX/1ER;->footer_:Ljava/lang/String;

    iput-object v0, v3, LX/1hN;->A05:Ljava/lang/String;

    iget-object v0, v4, LX/1Es;->description_:Ljava/lang/String;

    iput-object v0, v3, LX/1hN;->A04:Ljava/lang/String;

    iget-object v0, v4, LX/1Es;->currencyCode_:Ljava/lang/String;

    iput-object v0, v3, LX/1hN;->A03:Ljava/lang/String;

    invoke-static {v0}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_3

    :try_start_1
    iget-object v0, v3, LX/1hN;->A03:Ljava/lang/String;

    new-instance v2, LX/35K;

    invoke-direct {v2, v0}, LX/35K;-><init>(Ljava/lang/String;)V

    iget-wide v0, v4, LX/1Es;->priceAmount1000_:J

    invoke-static {v2, v0, v1}, LX/2vn;->A00(LX/35K;J)Ljava/math/BigDecimal;

    move-result-object v0

    iput-object v0, v3, LX/1hN;->A0A:Ljava/math/BigDecimal;

    iget-object v0, v3, LX/1hN;->A03:Ljava/lang/String;

    new-instance v2, LX/35K;

    invoke-direct {v2, v0}, LX/35K;-><init>(Ljava/lang/String;)V

    iget-wide v0, v4, LX/1Es;->salePriceAmount1000_:J

    invoke-static {v2, v0, v1}, LX/2vn;->A00(LX/35K;J)Ljava/math/BigDecimal;

    move-result-object v0

    iput-object v0, v3, LX/1hN;->A0B:Ljava/math/BigDecimal;

    goto :goto_0
    :try_end_1
    .catch Ljava/lang/IllegalArgumentException; {:try_start_1 .. :try_end_1} :catch_1

    :catch_1
    const/4 v0, 0x0

    iput-object v0, v3, LX/1hN;->A03:Ljava/lang/String;

    :cond_3
    :goto_0
    iget-object v0, v4, LX/1Es;->retailerId_:Ljava/lang/String;

    iput-object v0, v3, LX/1hN;->A08:Ljava/lang/String;

    iget-object v0, v4, LX/1Es;->url_:Ljava/lang/String;

    iput-object v0, v3, LX/1hN;->A07:Ljava/lang/String;

    iget v0, v4, LX/1Es;->productImageCount_:I

    iput v0, v3, LX/1hN;->A00:I

    iget-object v1, v4, LX/1Es;->productImage_:LX/1FB;

    if-nez v1, :cond_4

    sget-object v1, LX/1FB;->DEFAULT_INSTANCE:LX/1FB;

    :cond_4
    iget-object v0, p0, LX/3S3;->A01:LX/2li;

    invoke-static {p1, v1, v3, v0}, LX/2li;->A00(LX/2sL;LX/1FB;LX/1hX;LX/2li;)V

    return-object v3

    :cond_5
    const/4 v0, 0x0

    invoke-static {v0}, LX/1zA;->A00(I)LX/1zA;

    move-result-object v0

    throw v0

    :cond_6
    const/4 v3, 0x0

    return-object v3
.end method
